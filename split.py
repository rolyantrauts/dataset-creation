...
def split(source_dir='./in', dest_dir='./out', testing_percent=0.05, validation_percent=0.10, label="kw"):
  ...
  if not os.path.exists(dest_dir + "/valid"):
    os.makedirs(dest_dir + "/valid")  
  if not os.path.exists(dest_dir + "/train/" + label):
    os.makedirs(dest_dir + "/train/" + label)    
  if not os.path.exists(dest_dir + "/test/" + label):
    os.makedirs(dest_dir + "/test/" + label)  
  if not os.path.exists(dest_dir + "/valid/" + label):
    os.makedirs(dest_dir + "/valid/" + label)  
  ...
  for wav in source:
    if count < testing_qty:
      shutil.move(wav, dest_dir + "/test/" + label + "/" + os.path.basename(wav))
    elif count < validation_qty + testing_qty:
      shutil.move(wav, dest_dir + "/valid/" + label + "/" + os.path.basename(wav))
    else:
      shutil.move(wav, dest_dir + "/train/" + label + "/" + os.path.basename(wav))
    count += 1
